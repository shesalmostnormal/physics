	...
	for i in size:
		for j in size:
			...
			Hmatrix[i][j] =  (norm(i)*norm(j))*(0.5*(1/m) * p_sum[i][j] \
				- coupling * r_sum[i][j] + sigma * l_sum[i][j])
...
def Golden(m,s):
	...
	f1 = Calculate_Energy(m,x1)
	f2 = Calculate_Energy(m,x2)
	f3 = Calculate_Energy(m,x3)
	f4 = Calculate_Energy(m,x4)
	...
	while x4-x1>accuracy:
		if f2[s]<f3[s]:
			...
			x3,f3[s] = x2,f2[s]
			x2 = x4 - (x4-x1)/z
			f2[s] = Calculate_Energy(m,x2)[s]
		else:
			x1,f1[s] = x2,f2[s]
			x2,f2[s] = x3,f3[s]
			x3 = x1 + (x4-x1)/z
			f3[s] = Calculate_Energy(m,x3)[s]
...
for j in range(len(mass)):
	...
	for i in range(state):
		alphalist.append(Golden(mass[j],i))
	mesonlist.append(Calculate_Energy(mass[j],alphalist[i]))
...
with open('mesontest.csv', 'wb') as f:
